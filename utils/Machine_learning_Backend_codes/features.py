...
def align_sequences(sgRNA, genomic_extended):
   ...
   #print(aligned_sgRNA, aligned_offtarget)
...
if __name__ == '__main__':
   ...
   parser.add_argument('-CTCAGCTGAGGTTGCTGCTG', required=True, help="sgRNA seq of 20 bases (without PAM)")
   parser.add_argument('-GGCCTCAGCTGAGGTTGCTGCTGTGGAAG', required=True, help="DNA target sequence with 3 additional bases at each end (total of 29 nucleotides)")
   ...
   try:
      ...
      assert sgRNA_seq_re is not None, "sgRNA sequence must be 20-nt long sequence of ACGTU nucleotides"
      full_dna_seq_re = re.search("[acgtu]+", full_dna_seq, re.IGNORECASE)
      assert full_dna_seq_re is not None, "genomic sequence must be 29-nt long sequence of ACGTU nucleotides"
   except:
   ...
   ### align_sequences
   aligned_sgRNA, aligned_offtarget, max_score = align_sequences(sgRNA="CTCAGCTGAGGTTGCTGCTG", genomic_extended="GGCCTCAGCTGAGGTTGCTGCTGTGGAAG")

   ### get features
   features = get_features(full_dna_seq="GGCCTCAGCTGAGGTTGCTGCTGTGGAAG", aligned_sgRNA=aligned_sgRNA, aligned_offtarget=aligned_offtarget,
                     pa_score=max_score)

   result1=(aligned_sgRNA,full_dna_seq,features)
   with open("featurelist_back.csv", 'a+') as myfile:
          myfile.write(str(result1)+ "\n")
   print(aligned_sgRNA,",",full_dna_seq,",",features)
